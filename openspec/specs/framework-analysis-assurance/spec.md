# framework-analysis-assurance Specification

## Purpose
How a framework drawn from a book is applied to a question, for any domain. It requires the AI to test whether the selected framework applies, to keep source material, external facts, and inference distinct, to record cross-cutting challenges and uncertainty, and to state a conditional conclusion bounded by the evidence rather than a prescribed decision template.
