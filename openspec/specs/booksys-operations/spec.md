# booksys-operations Specification

## Purpose
The command surface through which a skill reaches the book catalog and the learning record. It resolves system and data roots from explicit options or `.booksys/config.json`, offers read-only health checks, and locates declared books, chapters, and concepts by stable identifier only -- never by title, alias, ranking, or semantic guess.
